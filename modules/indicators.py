...
def compute_indicators(data, indicators, params):
    ...
    if indicators.get('MA'):
        ...
        threshold = params.get('ma_threshold', 0.01)
    ...
    if indicators.get('MACD'):
        ...
        threshold_macd = params.get('macd_threshold', 0.1)
    ...
    # Ichimoku (tidak pakai threshold tambahan untuk saat ini)
    ...
    if indicators.get('SO'):
        ...
        threshold_so = params.get('so_threshold', 2.0)
    ...
    # === PREDICTIVE SIGNAL (Buy jika besok naik, Sell jika besok turun) ===
    data['Future_Close'] = data['Close'].shift(-1)
    price_change_pct = ((data['Future_Close'] - data['Close']) / data['Close']) * 100

    threshold_buy = params.get('predictive_buy_threshold', 1.0)
    threshold_sell = params.get('predictive_sell_threshold', -1.5)

    data['Predictive_Signal'] = 'Hold'
    data.loc[price_change_pct > threshold_buy, 'Predictive_Signal'] = 'Buy'
    data.loc[price_change_pct < threshold_sell, 'Predictive_Signal'] = 'Sell'
